...
const News = () => {
  ...
  useEffect(() => {
    const fetchNews = async () => {
      ...
      const res = await fetch(
        `${process.env.API_URL || "http://115.79.212.239:3000:3009"}/api/news`
      );
  ...
  const handleDelete = async (id) => {
    const res = await fetch(
      `${
        process.env.API_URL || "http://115.79.212.239:3000:3009"
      }/api/news/${id}`,
      {
        method: "DELETE",
      }
            ...
            <div className="bg-slate-300 p-4 rounded">
              ...
              {news.file && (
                <div className="mb-2 w-[200px] h-[200px]">
                  <strong>Image:</strong>
                  <img
                    src={`http://115.79.212.239:3000:3009/${news.file}`} // Đảm bảo rằng bạn trả về đúng URL hình ảnh
                    alt={news.title}
