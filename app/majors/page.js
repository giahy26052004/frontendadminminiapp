...
const Majors = () => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        const res = await fetch(`http://115.79.212.239:3009/api/majors/${id}`, {
          method: "DELETE",
        });
        if (res.ok) {
        ...
        {majors.map((major) => (
          ...
          >
            ...
            <p className="text-gray-600">
              <strong>Khối ngành:</strong> {major.khoi_nganh}
            </p>
            <p className="text-gray-600">
              <strong>Kiến trúc:</strong> {major.details_nganh.kien_truc}
            </p>
            <p className="text-gray-600">
              <strong>Mục tiêu:</strong> {major.details_nganh.muc_tieu}
            </p>
            <p className="text-gray-600">
              <strong>Cơ hội nghề nghiệp:</strong>{" "}
              {major.details_nganh.co_hoi_nghe_nghiep}
            </p>
            <p className="text-gray-600">
              <strong>Điều kiện tuyển sinh:</strong>{" "}
              {major.details_nganh.dieu_kien_tuyen_sinh}
            </p>
            <p className="text-gray-600">
              <strong>Môn học tiêu biểu:</strong>{" "}
              {major.details_nganh.mon_hoc_tieu_bieu.join(", ")}
            </p>
            <p className="text-gray-600">
              <strong>Bằng cấp:</strong> {major.details_nganh.bang_cap}
            </p>
